...
public class InversionMutation extends HeuristicOperators implements HeuristicInterface {
	...
	public double apply(SDSSTPSolutionInterface solution, double depthOfSearch, double intensityOfMutation) {
		...
		for (int i = 0; i < iTimes; i++) {
			int iLocationa = random.nextInt(iLandmarks);
			int iLocationb = iLocationa;
			// get different 2 location
			while(iLocationa == iLocationb) {
				iLocationb = random.nextInt(iLandmarks);
			}
			int temp = iRepresentation[iLocationa];
			iRepresentation[iLocationa] = iRepresentation[iLocationb];
			iRepresentation[iLocationb] = temp;	
