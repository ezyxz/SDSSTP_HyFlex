...
public class NextDescent extends HeuristicOperators implements HeuristicInterface {
	...
	public double apply(SDSSTPSolutionInterface solution, double depthOfSearch, double intensityOfMutation) {
		...
		while(iTimes > 0 && iRunTime < iLandmarks) {
			if(iRunTime == iLandmarks - 1) {
				int[] swap_iTempRepresentation = iTempRepresentation.clone();
				int temp = swap_iTempRepresentation[iRunTime];
				swap_iTempRepresentation[iRunTime] = swap_iTempRepresentation[0];
				...
				int[] swap_iTempRepresentation = iTempRepresentation.clone();
				int temp = swap_iTempRepresentation[iRunTime];
				swap_iTempRepresentation[iRunTime] = swap_iTempRepresentation[iRunTime+1];
				swap_iTempRepresentation[iRunTime+1] = temp;
				int delta = valueByDeltaEvaluation(iTempRepresentation, swap_iTempRepresentation,0,iLandmarks);
				if( delta < 0 ) {
					iTempRepresentation = swap_iTempRepresentation;
					iTimes--;
...
//	        int ans = getObjectiveFunction().getObjectiveFunctionValue(solution.getSolutionRepresentation());
//			System.out.println(iFunctionValue == ans);
